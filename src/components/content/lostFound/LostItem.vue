    ...
    <div id="lost-item">
        <div v-if="Object.keys(lostFounditem).length != 0">
            ...
            <p class="phone" v-show="lostFounditem.telphone != null">
                ...
                <span> &nbsp;{{ lostFounditem.telphone }} </span>
                <span
                    v-clipboard:copy="lostFounditem.telphone"
                    v-clipboard:success="onCopy"
                    v-clipboard:error="onError"
                    class="copy-phone"
                    v-if="isShowCopyPhone"
                    >一键复制号码</span
                >
            </p>
            <!-- 时间 -->
            <p class="phone" v-show="lostFounditem.findTime != null">
                ...
                <span> &nbsp;{{ lostFounditem.findTime }} </span>
            </p>
            <!-- 位置 -->
            ...
            <!-- 配图 -->
            <ul class="lost-imaes" v-show="lostFounditem.images != null">
                <li
                    v-for="(image, index) in lostFounditem.images"
                    :key="index"
                    @click="getComment(index)"
                >
                    <img v-lazy="image" />
                </li>
            </ul>
            <div>
                        ...
                        <p class="phone">
                            <span
                                ><van-icon
                                    name="comment-o"
                                    size="25px"
                                    @click="getComment" /></span
                            >&nbsp;
                            <span>{{ CommentsLength }}</span>
                        </p>
                    </li>
                    <li></li>
                    <li class="phone-detail" @click="getComment">
                        <span>点击查看详情>>></span>
                    </li>
                </ul>
                <!-- 评论部分开始 -->
                <div v-else class="commentcount">
                    ...
                    <div>
                        <span v-if="lostFoundItemComments != 0"
                            >全部评论&nbsp;({{ CommentsLength }})</span
                        >
                        <span v-else>全部评论&nbsp;(0)</span>
                    </div>
                    <ul v-show="CommentsLength != 0">
                        <li
                            v-if="mainArr.length != 0"
                            v-for="(i, index0) of mainArr"
                            :key="index0"
                            class="border-comments van-hairline--top"
                        >
                                ...
                                <li>
                                    ...
                                    >
                                        <p
                                            v-for="(
                                                itemm, indexx
                                            ) of commentArr[index0]"
                                            @click="reply(index0, indexx)"
                                            v-if="commentArr[index0].length > 0"
                                            :key="indexx"
                                        >
                                            {{
                                                commentArr[index0][indexx][0]
                                                    .user.name
                                            }}
                                            <span style="color: #00ac2c"
                                                >回复</span
                                            >
                                            {{
                                                commentArr[index0][indexx][1]
                                                    .user.name
                                            }}:&nbsp;{{
                                                commentArr[index0][indexx][0]
                                                    .content
        ...
        >
            <div class="content">
                ...
                <van-button
                    ...
                    color="#00ac2c"
                    class="btn"
                    @click="commentAxios(indexComment)"
                    >发送</van-button
                >
            </div>
        </van-action-sheet>
...
export default {
    ...
    watch: {
        ...
        $route: {
            handler: function (route) {
                ...
                if (route.path == "/lostFoundItem") {
                    this.isShowCopyPhone = true;
                }
            },
            immediate: true,
        },
    ...
    created() {
        ...
        if (this.$route.path == "/lostFoundItem") {
            this.isShowCopyPhone = true;
        } else {
            this.isShowCopyPhone = false;
        }
    },
    mounted() {
        console.log(this.lostFounditem);
    ...
    methods: {
        ...
        async getCommentsById(x) {
            ...
            await this.$axios
                ...
                .then((res) => {
                    ...
                    // 时间转换
                    this.lostFoundItemComments.forEach((item) => {
        ...
        commentGOhome1() {
            ...
            this.commentArr = [];
            if (
                this.lostFoundItemComments.length == 0 ||
                this.lostFoundItemComments == null ||
                this.lostFoundItemComments == undefined
            )
                return;
            ...
            this.mainArr.forEach((item, index) => {
                ...
                // 新增开始
                let arrcomments = this.lostFoundItemComments.filter((item) => {
                    return item.commentId != null;
                });
        ...
        getComment(x) {
            if (this.$route.path == "/lostFoundItem") {
                ImagePreview({
                    images: this.lostFounditem.images,
                    startPosition: x,
                    showIndicators: true,
                    closeable: true,
                ...
                this.$router.push({
                    path: "/lostFoundItem",
                    query: { id: this.lostFounditem.id, isShowItem: false },
                });
            }
        },
        // 操作启事动态
        ...
        reply(x, y) {
            ...
            this.commentId = this.commentArr[x][y][0].id;
